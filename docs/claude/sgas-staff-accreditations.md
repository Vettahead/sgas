...
## UI
NEW `src/components/StaffAccreditations.jsx` (props `staffId, staffName, onCount`). Admin.jsx gained an **Accreditations column** (pill button, "N held" + amber/red count dots) between Holidays and Login; clicking opens a full-width `tr.accrow` underneath. Admin.jsx also gained a module-level `summarise(rows)` helper and `accFor`/`accCounts` state. Table colSpans went 8→9.
