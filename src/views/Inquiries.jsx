...
// openId: { inquiryId, at } from a notification click — open that one on arrival.
export default function Inquiries({ go, user, onMentionsSeen, openId = null }) {
  ...
  // Arrived from a notification: open that enquiry as soon as the list is
  // here, on whichever tab it lives. `at` makes a second click on the same
  // enquiry open it again.
  const handledOpen = useRef(null)
  useEffect(() => {
    if (!openId || !inquiries || handledOpen.current === openId.at) return
    const q = inquiries.find((x) => x.inquiryId === openId.inquiryId)
    if (!q) return
    handledOpen.current = openId.at
    setTab(q.status || 'open')
    openInq(q)
