          ...
          {visible.map((b) => {
            ...
            return (
              <div key={b.id} className={'bcard' + (b.id === selectedId ? ' on' : '') + (dropping ? ' droptarget' : '')}
                ...
                <div className="bc-delg small">
                  <span className="muted">👤 {b.delegates.length}</span>
                  {b.delegates.slice(0, 5).map((d) => <span key={d.bookingId} className="bc-av" style={{ background: kindColor(d.kind) }} title={d.name}>{initials(...d.name.split(' '))}</span>)}
                  {b.delegates.length > 5 && <span className="muted">+{b.delegates.length - 5}</span>}
                </div>
              </div>
            )
          })}
        ...
        {sBlock && (
          <div className={'block-drawer' + (sel?.type === 'delegate' ? ' droptarget' : '')}
            ...
            <div className="drawer-head">
              <span className="bc-name">📚 {sBlock.course}</span>
