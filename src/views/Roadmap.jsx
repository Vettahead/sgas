...
const ITEMS = [
  ...
  { t: 'Sage — one question left', s: 'simon', d: 'Two things needed checking and Chris has closed the first. Jen is on Sage Accounting Plus (GB) — the cloud version, so this system can talk to it directly rather than needing a computer in the office left switched on permanently to act as a go-between — and Chris confirmed 29 Aug that the connection IS included on that plan, so there is no upgrade to buy. One thing left to ask, and it is not technical: whose Sage login would authorise the connection, and are they happy for it to be used that way? Read-only to begin with, so nothing here can alter a real invoice. Once that comes back this can be properly costed.' },
  ...
  { t: 'Sage integration', id: 'sage', s: 'future', d: 'Sage Business Cloud Accounting has a REST API with OAuth sign-in, so Jen would sign into the system with her Sage details and the two talk to each other. Bookings generate a draft invoice in Sage for Jen to check and release; payments come back the other way. Money lives in Sage, so the items nested below hang off this — if Sage cannot do it, they fall away. The decision to make first is which way round it runs: either a booking raises a draft invoice in Sage for Jen to check and release, or — far smaller — Jen carries on raising invoices exactly as she does now and we only read back whether something has been paid, ticking Paid automatically. The second changes nobody’s routine and is the one to price first. Waiting on the two questions above.' },
