...
const ITEMS = [
  ...
  { t: 'Sage — two questions before it can be costed', s: 'simon', d: 'Chris confirmed 29 Aug that Jen is on Sage Accounting Plus (GB). That is the good answer: it is the cloud version, so it has a proper connection built in and this system can talk to it directly. Sage 50 would have meant a computer in the office left switched on permanently just to act as a go-between. TWO THINGS TO ASK JEN, neither of them technical. First, is that connection included on the Plus plan, or does it need a step up? Sage has charged for it by plan before now. Second, whose Sage login would authorise it, and are they happy for it to be used that way — ideally read-only to begin with, so nothing here can alter a real invoice. Nothing can be quoted or built until both come back.' },
