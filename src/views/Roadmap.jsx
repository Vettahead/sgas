...
const ITEMS = [
  ...
  { t: 'Access database import', s: 'progress', d: 'IN PROGRESS — the matching worklist is finished. Simon has now answered all 277 rows: 113 qualification columns matched to ours and 9 left out, eleven staff spellings resolved to six people, and 109 employers to be created. Two questions were settled on 6 Sep by going back to the file itself. The assessors nobody recognised — G Nutton, R Darlow, the Crossdales — look enormous on the worklist because those counts cover all twenty years; inside the seven years being imported they account for eight records between them, so leaving them out costs nothing. And “Assesment only”, which the file records against 3,244 courses, turns out to be a habit from 2000 to 2003 that stopped: eleven since 2014, and the trainer column is filled on 23 of the 4,817 records in scope. It is being built as a proper booking option instead of imported. NEW 6 Sep: a Create button on the Data import tab now turns those answers into real records — until now confirming a row only wrote down the answer, and the 109 employers existed nowhere. Still to build: the load itself — pulling the 4,817 records across, working out which of them are the same person, and holding back the ones that cannot be told apart for somebody to look at.' },
