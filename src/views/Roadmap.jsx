...
const ITEMS = [
  ...
  { t: 'Planner — the experimental calendar', s: 'review', d: 'BUILT 27 Aug 2026, as a new tab beside the existing Calendar, which is untouched. One timeline instead of four separate views: days are columns and zooming just changes how wide a day is, so sliding from a whole year down to a single day grows the course bars smoothly rather than redrawing the screen — you never lose your place. Alongside it sits everything you would otherwise have to go looking for: delegates waiting to be placed, courses needing attention (no trainer, no delegates, trainer on holiday), and the staff list. Everything works two ways — drag it across, or tap it and then tap a course, which is quicker on a tablet and far easier to pick up. Bars can be dragged to move and pulled at the edge to change length. Built on pointer events throughout, so unlike the current calendar it works on a tablet. Switch to “By trainer” to see each person’s row and who has nothing on. RESEARCHED FIRST: 3D (Three.js) was ruled out — it renders small text badly and is invisible to screen readers; the animation is done with plain CSS, so nothing was added to the app.' },
