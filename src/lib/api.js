...
export async function listMyMentions(user = null) {
  if (LIVE) {
    try {
      ...
      if (error) return []
      return (data || []).map((m) => ({
        messageId: m.message_id, inquiryId: m.inquiry_id, createdAt: m.created_at,
        author: m.author_name || 'Someone', body: m.body || '', inquiryName: m.inquiry_name || '',
      }))
    } catch { return [] }
  }
  return demoMsgs().filter((m) => (m.mentions || []).includes(user?.user_id) && !(m.seenBy || []).includes(user?.user_id))
    .map((m) => ({
      messageId: m.message_id, inquiryId: m.inquiry_id, createdAt: m.created_at,
      author: m.author_name, body: m.body, inquiryName: D.inquiries.find((i) => i.inquiry_id === m.inquiry_id)?.name || '',
    }))
}
...
// `kind` says WHERE the person is (see src/lib/whereabouts.js); `endDate` and
// `half` exist because a quarter of these run over several days and a handful
...
export async function createEngagement({
  ...
  if (endDate && endDate < date) throw new Error('That ends before it starts')
  const members = (memberStaffIds || []).map(Number).filter(Boolean)
  if (LIVE) {
    const { data, error } = await supabase.from('engagement').insert({
