-- ─────────────────────────────────────────────────────────────────────────────
-- Promotion, third pass: one seat per person per course.
--
-- With delegates linked by SGAS number, the same person can arrive at one event
-- twice — a name written on two lines, or two of their bookings pointing at the
-- same day. A person sits a course once, so take the booking whose assessment
-- date is nearest the day and leave the rest alone.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- Only rows this import created are ever removed, and a session with a
  -- delegate on it is never removed at all: the booking wins and the event goes
  -- to somebody to look at instead.
  with gone as (
  ...
  -- Whereabouts, meetings, site visits and the days the centre is shut. The
  -- person goes on as a MEMBER. A closed day has no member: it is nobody's.
  -- Anything still 'unknown' is deliberately NOT brought across — a day we
  -- cannot read does not belong on the calendar as a guess.
  with who as (
    select e.event_id, e.start_dt::date ds, coalesce(e.end_dt::date, e.start_dt::date) de,
  ...
  -- A person sits a course once. With links coming from SGAS numbers the same
  -- person can arrive twice — written on two lines, or two of their bookings
  -- pointing at one day — so take the booking nearest the day and leave the
  -- others where they are. A booking already on a session is left alone:
  -- something deliberate beats something derived.
  with pick as (
    select distinct on (s.session_id, b.client_id) b.booking_id, s.session_id
      from teamup_event_delegate l
      join session s on s.teamup_event_id = l.event_id
      join booking b on b.booking_id = l.booking_id
     where l.confidence = 'certain' and not l.name_shared and b.session_id is null
     order by s.session_id, b.client_id, abs(coalesce(l.day_gap, 999))
  )
